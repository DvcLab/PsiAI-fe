...
export default {
  ...
  methods: {
    ...
    initWebSocket(){
      ...
      const wsuri = process.env.VUE_APP_WS_URL + "_host_info?host_id=" + this.host.id
    ...
    <!-- <div class="card">
      <div class="card-body"> -->
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="row mb-1">
              <div v-if="!this.$_.isNil(hostSelfData.cpu_info.cpu_model_name)" class="col-sm-12 col-md-8 mb-2" style="font-weight: bold;">
                {{hostSelfData.cpu_info.cpu_model_name}}
              </div>
              <div v-if="!this.$_.isNil(hostSelfData.cpu_info.cpu_num)" class="col-sm-6 col-md-4 mb-2">
                <span class="badge rounded-pill bg-primary">
                  <i class="uil uil-circuit" />内核
                </span> {{ hostSelfData.cpu_info.cpu_num }}
              </div>
              <div v-if="!this.$_.isNil(hostSelfData.mem_info.total)" class="col-sm-6 col-md-4 mb-2">
                <span class="badge rounded-pill bg-primary">
                  <i class="uil uil-dice-four" />内存
                </span> {{ hostSelfData.mem_info.total }} MiB
              </div>
              <div v-if="!this.$_.isNil(hostSelfData.container_num)" class="col-sm-6 col-md-4 mb-2">
                <span class="badge rounded-pill bg-primary">
                  <i class="uil uil-box" />容器
                </span> {{ hostSelfData.container_num }}
              </div>
              <div v-if="!this.$_.isNil(hostSelfData.gpu_info)" class="col-sm-6 col-md-4 mb-2">
                <span class="badge rounded-pill bg-primary">
                <i class="uil uil-server" />Nvidia Driver {{ hostSelfData.gpu_info.driver_version }}</span> 
              </div>
              <div v-if="!this.$_.isNil(hostSelfData.gpu_info)" class="col-sm-6 col-md-4 mb-2"><span class="badge rounded-pill bg-primary">
                <i class="uil uil-servers" /> CUDA {{ hostSelfData.gpu_info.cuda_version }}</span>
              </div>
            </div>
          </div>
            ...
            <div class="row">
              ...
              <span class="col-sm-3 col-md-3">
                <apexchart
                  ...
                  :options="getChartOptions(hostSelfData.cpu_info.max_temp,'CPU temp','℃')"
                ></apexchart>
              </span>
              <span class="col-sm-3 col-md-3">
                <apexchart
